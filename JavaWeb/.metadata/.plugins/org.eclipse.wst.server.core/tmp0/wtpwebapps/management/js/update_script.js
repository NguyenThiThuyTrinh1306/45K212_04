changeName = () => {
	...
	const url = "http://localhost:8080/management/admin?newName=" + name;
	if (name == "") {
...
changePassword = () => {
	...
	const url = "http://localhost:8080/management/admin?currentPassword=" + cur + "&newPassword=" + newP;
	if (newP == "" || cur == "") {
		...
		.then(res => res.json())
		.then(data => {
			const color = (data.status == true) ? "#2980b9" : "#e74c3c";
